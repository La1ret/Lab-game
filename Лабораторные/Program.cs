...
internal class Programm
{
    ...
    public static int TowerLocation()
    {
        ...
        while (!exit2)
        {
            ...
            if (newY == 5 && newX == 0) { exit2 = true; Console.Clear(); PlayerX = 15; PlayerY = 4; StartLocation(); }//выход в стартовую локу
    ...
    public static string StartLocation()
    {
        ...
        while (!exit1)
        {
            ...
            if (newX >= 0 && newX < map.GetLength(1) && newY >= 0 && newY < map.GetLength(0) && (map[newY, newX] == ' ' || map[newY, newX] == '▒'))//условие передвижения
            {
                PlayerX = newX;
                PlayerY = newY;
            }

            if ((newX == 7 || newX == 8) && newY == 4)//подход к камню
            {
            ...
            else if ((PlayerX == 0 || PlayerX == 15) && PlayerY == 4)//выход из локи
            {
            ...
            while (isGameOver == false)
            {
                ...
                if (Player.Health <= 0)
                {
                    if (Player.HeallCount > 0)
                    {
                        ...
                        Console.WriteLine($"\nВы съедаете ягоды и чувствуете прилив сил!\n{Player.Name} расходует Ягоды ({Player.HeallCount} шт.) и восстанавливает { Player.HeallCount * 3}  НР");
                        Console.ReadKey();
                    }
                    else
                    {
